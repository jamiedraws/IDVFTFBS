...
<% var ProductName = SettingsManager.ContextSettings["Label.ProductName"]; %>
	...
	<div class="connect u-pad--vert @x3-pad center-text white-background @back-only-hide">
		<h2 class="color-gray uppercase sans-secondary-semibold u-pad--vert">
			Connect. Follow. Discover.
		</h2>
		<h4 class="color-tertiary sans-secondary-semibold">@fintouchflawless</h4>
		<div class="color-gray h4"><% Html.RenderPartial("Social", Model); %></div>
	...
	<footer class="l-footer bg-gray tertiary-container o-box--none block u-pad @print-only-hide">

		<nav class="l-footer__in white-text c-brand @mv-u-pad--reset fn--center u-mar--center">

			<img src="/images/2.0000/logo--white.png" alt="<%= SettingsManager.ContextSettings["Label.ProductName"] %>">
	...
	<!-- needed for before after slider -->
	...
	<script>
		...
		(function () {
			// part of intersection observer
			// get html

			/*

			note to Jamie: started to incorporate intersection observer but stopped

			var $html = $('html');
			// set ui to false
			var userInitiated = false;

			// get media player container
			var mediaPlayer = document.querySelector('.media__content');
			// capture when vimeo iframe has loaded onto the DOM
			var observer = new MutationObserver(function (mutationList) {
				// get vimeo iframe
				var vimeo = mutationList.map(function (obj) { return obj.addedNodes[0]; });
				// sync to Vimeo api
				_dtm.getVimeo($(vimeo));

			});

			// observe the media player container for a DOM change
			...
			*/
			...
			function getProduct (item, code, prop, sum) {
				var $defer = new $.Deferred();

				$.post('/GetCategory', { item : item, code : code, prop : prop, sum : sum }, function (response) {
					try {
			...
			$(window).ready(function () {
				...
				getProduct(null, 'QUOTES', null, true).done(function (len) {
					for ( var i = 0; i < len; i++ ) {
						if ( i === len - 1 ) {
							$quotes.trigger({ type : 'ready/quotes:data', len : len });
						} else {
							getProduct(i, 'QUOTES').done(function (data) {
								buildQuote(data);
							});
						}
					}
	...
	// Instagram
	 ...
	 var feed = new Instafeed({
       get: 'user',
       userId: 1537743608,
       accessToken: '1537743608.7d0313e.9dc7f6c260f24009a7c4a7da2f5473f3',
       clientId: '',
