<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<ClientSiteViewData>" %>
	...
	<link rel="stylesheet" href="/css/customer-cart-status/app.css">
	<script src="/js/app.es5.js" defer></script>
		...
		<section id="customer-cart-status" data-modal-dialog-container data-modal-dialog-title="Customer Cart Status" aria-labelledby="customer-cart-status-title" class="view customer-cart-status section">
			<div class="view__in customer-cart-status__in section__in">
				<div class="customer-cart-status__group">
					<div id="customer-cart-status-title" class="customer-cart-status__response">Added To Cart</div>
					...
					<div class="customer-cart-status__action">
						<a href="/ShoppingCart.dtm#review" class="button customer-cart-status__button">
							<span>Checkout</span>
						</a>
					</div>
				</div>
	...
	<%
            ...
            var isBlogPage = blogPages.Contains(DtmContext.PageCode);
	...
	<script>
        addEventListener("CartChange", function () {
			...
			(function () {
				...
				$('.remove-zoom').click(function() {
					$('.gallery__main').removeClass('has-zoom');
				});

				$('.add-zoom').click(function() {
					$('.gallery__main').addClass('has-zoom');
				});
				...
				$('html').on('dtm/vimeo', function () {
					_dtm.makeVimeoResponsive();
							...
							$(document).ready(function () {
                                ...
                                toggleFlawlessImages();
								$( "[name=singleDropdown_GDQ_FLAWLESS6]" ).change(function() {
                                    toggleFlawlessImages();
									});
                ...
                function toggleFlawlessImages(){
                    var value = $("select[name='singleDropdown_GDQ_FLAWLESS6']").val();
                            if (value === 'BLUSH') {
							    document.getElementById('gallery__main').src="/images/products/3.0000/flawless--color-home.png";
						    }else if (value === 'BLACK2') {
							    document.getElementById('gallery__main').src="/images/products/14.0000/flawless--black.png";
						    }
                }
		...
		<script>
            ...
            const vseError = document.querySelector("#vse-errors");

            _dtm.callbackAlert = function (errors, $vse) {
                if (vseError) {
                    vseError.scrollIntoView();
                }
            }
        </script>
			<% }
